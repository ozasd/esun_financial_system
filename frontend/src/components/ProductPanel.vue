...
<template>
  <section class="panel fade-in-up">
    <div class="panel-header">
      <div><h2>📦 商品管理</h2><p class="panel-desc">管理金融商品資訊</p></div>
      <button class="btn btn-primary" @click="openAdd">＋ 新增商品</button>
    </div>
    <div class="toolbar">
      <div class="search-box">
        <input v-model="keyword" placeholder="搜尋商品名稱…" @keyup.enter="doSearch" />
      ...
      <div class="toolbar-right">
        <select v-model="sortBy" @change="doSearch">
          ...
          <option value="fee_rate">手續費率</option>
        </select>
    ...
    <div v-else class="table-wrap">
      <table>
        <thead><tr><th>編號</th><th>商品名稱</th><th>價格</th><th>手續費率</th><th>建立時間</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-if="products.length === 0"><td colspan="6" class="empty">尚無資料</td></tr>
          <tr v-for="p in products" :key="p.no">
            <td class="mono">{{ p.no }}</td>
            <td><span class="product-tag">{{ p.productName }}</span></td>
            <td class="price-cell">${{ formatPrice(p.price) }}</td>
            <td><span class="rate-badge">{{ formatRate(p.feeRate) }}</span></td>
            <td class="text-muted">{{ p.createdAt?.substring(0, 10) }}</td>
            <td>
              <button class="icon-btn" @click="openEdit(p)">✏️</button>
              <button class="icon-btn danger" @click="deleteProduct(p.no)">🗑️</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <Teleport to="body">
      <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
        <div class="modal fade-in-up">
          <h3>{{ editMode ? '編輯商品' : '新增商品' }}</h3>
          <div v-if="formError" class="form-error">{{ formError }}</div>
          <form @submit.prevent="submitForm" class="form-grid">
            <label><span>商品名稱</span><input v-model="form.productName" required placeholder="例：台股基金" /></label>
            <label><span>價格</span><input v-model="form.price" type="number" min="0" step="0.01" required placeholder="例：10000" /></label>
            <label><span>手續費率</span><input v-model="form.feeRate" type="number" min="0" max="1" step="0.001" required placeholder="例：0.01（表示 1%）" /></label>
...
.product-tag { font-weight:600; color:var(--color-text); }
.price-cell { font-weight:600; color:var(--color-primary-dark); font-family:var(--font-mono); }
.rate-badge { display:inline-block; background:var(--color-accent-bg); color:var(--color-accent-dark); padding:2px 8px; border-radius:var(--radius-full); font-size:0.78rem; font-weight:600; }
