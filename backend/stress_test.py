...
async def run_test(name, url, method="GET", get_data_func=None, get_headers_func=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def bounded_fetch():
            async with sem:
                ...
                headers = get_headers_func() if get_headers_func else None
                return await fetch(session, url, method, data, headers)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        await fetch(session, f"{BASE_URL}/api/favorite-products/users/A1236456789")
    ...
    def valid_payload():
        return {
            "userId": "A1236456789",
            "productNo": random.randint(1, 5),
            "purchaseQuantity": random.randint(1, 10),
            "account": "1111999666"
        }

    def unique_idempotency_header():
        return {
            "Idempotency-Key": f"stress-{time.time_ns()}-{random.randint(1000, 9999)}"
        }
    ...
    await run_test(
        ...
        method="POST",
        get_data_func=valid_payload,
        get_headers_func=unique_idempotency_header
    )
